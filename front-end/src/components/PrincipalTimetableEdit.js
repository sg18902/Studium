...
const PrincipalTimetableEdit = ({ open, onClose, classroomid }) => {
    ...
    useEffect(() => {
        if (Array.isArray(timetable) && timetable.length > 0) {
            const scheduleData = timetable[0]?.schedule;
            if (Array.isArray(scheduleData) && scheduleData.length > 0) {
                setSchedule(scheduleData);
            } else {
                setSchedule([{ subject: '', day: '', startTime: '', endTime: '' }]);
                // setError('Invalid or empty schedule');
            }
        } else {
            setSchedule([{ subject: '', day: '', startTime: '', endTime: '' }]);
    ...
    return (
        <Modal open={open} onClose={onClose}>
            <Box sx={{ p: 4, backgroundColor: 'white', margin: 'auto', marginTop: '10%', width: '60%', borderRadius: 2 }}>
                ...
                {timetable[0]?.schedule && timetable[0]?.schedule.length > 0 ?
                <>
                ...
                : 
                <>
                <Typography>The timetable has not been created yet.</Typography>
                </>
...
export default PrincipalTimetableEdit;
